...
def save_model_if_best(model, best_loss, eval_loss, args, custom_config):
    """Save the model if it achieves a new best (lowest) validation loss."""
...
def process_batch(model, data_loader, prior_box_s_gpu, criterion, optimizer, train=True):
    """Process a training or evaluation batch."""
    ...
    for step, (image_s_cpu, box_ss_cpu, label_ss_cpu) in bar:
        ...
        if all(len(l) > 0 and len(b) > 0 for l, b in zip(label_ss_gpu, box_ss_gpu)):
            with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                pred_loc_ss_gpu, pred_conf_ss_gpu = model(input['template'], input['search'])
...
def evaluate_model(model, dataloader, map_metric, prior_box_s, custom_config):
    """Evaluate the model and compute the mean average precision."""
    model.eval()
    map_metric.reset()
    ...
    with torch.no_grad():
        for step, (image_s_cpu, box_ss_cpu, label_ss_cpu) in bar:
            ...
            if all(len(l) > 0 and len(b) > 0 for l, b in zip(label_ss_gpu, box_ss_gpu)):
                # with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                pred_locs, pred_confs = model(input['template'], input['search'])
...
def prepare_input(image_s_cpu, k, dct):
    """Prepare and return model input dictionary."""
    # print(k)
    # print([dct[el[-1].item()] for el in k])
    # print(tokenizer.tokenize([dct[el[-1].item()] for el in k], context_length=225))
    return {
        'template': tokenizer([dct[el[-1].item()] for el in k], return_tensors='pt',
                              padding='max_length',
                              max_length=81,
                              truncation=True, ).to('cuda'),
        'search': image_s_cpu.cuda(),
        'label_cls': None,
        'label_loc': None,
...
def train_process(model, dataloaders, args, custom_config, logger=None, experiment_name=''):
    ...
    # optimizer = torch.optim.AdamW(model.parameters(), lr=args.learning_rate * 0.1, weight_decay=args.weight_decay)
